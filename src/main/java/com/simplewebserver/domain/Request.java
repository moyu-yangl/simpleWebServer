...
import static com.simplewebserver.connector.Constants.*;
...
public class Request {
    ...
    public Request(byte[] data) throws RequestParseException, RequestInvalidException, IOException {
        ...
        try {
            //支持中文，对中文进行URL解码
            lines = URLDecoder.decode(new String(data, UTF_8_CHARSET), UTF_8).split(CRLF);
...
//        session = servletContext.createSession(requestHandler.getResponse());
    ...
    private void parseBody(String body) {
        ...
        if (lengths != null) {
            ...
            parseParams(new String(bytes, 0, Math.min(length, bytes.length), UTF_8_CHARSET).trim());
        } else {
